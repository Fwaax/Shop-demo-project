...
const BuyItemFromShopModal = (props: { storeObjId: string, priceFromFE: number, hide: () => void }) => {
    ...
    async function handleBuyItem() {
        try {
            const response = await axios({
                method: 'put',
                url: `http://localhost:7821/shop/buy-item`,  // Fix later not currect url for the posting the item to shop
                headers: {
    ...
    return (
        <div className="flex flex-col gap-0 bg-[#111111] max-w-full w-[440px] rounded-[16px] border-2 border-solid border-white/6">
            ...
            <FieldsSection priceFromFE={props.priceFromFE} insertedPriceValue={insertedPriceValue} setInsertedPriceValue={setInsertedPriceValue} insertedQuantityValue={insertedQuantityValue} setInsertedQuantityValue={setInsertedQuantityValue} />
            <BottomSection hide={props.hide} onPostBTNClick={handleBuyItem} />
...
function TopSection({ hide }: { hide: () => void }) {
    return (
        <div className="flex flex-row justify-between items-center px-4 py-4 border-b border-solid border-white/6">
            <p className="font-vt323 text-[20px] text-white [word-spacing:6px]">Buy Item</p>
            <span
...
function FieldsSection(props: { priceFromFE: number, insertedPriceValue: number, setInsertedPriceValue: (value: number) => void, insertedQuantityValue: number, setInsertedQuantityValue: (value: number) => void }) {
    return (
        <div className="flex size-full w-[430px] flex-col gap-10 p-5">
            <div className="flex flex-col gap-4 p-2">
                <p className="font-vt323 text-base text-white/50">Price</p><p className="font-vt323 text-base text-white/50">{props.priceFromFE}</p>
...
export default BuyItemFromShopModal
